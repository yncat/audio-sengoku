...
class SingletonWindow():
	...
	def frameUpdate(self):
		...
		if self.keyPressed(keyCodes.K_LCTRL): self.sayStop()
		if self.keyPressing(keyCodes.K_LALT) and self.keyPressed(keyCodes.K_F4): self.exit()
		for event in pygame.event.get():
			if event.type == keyCodes.QUIT: self.exit()
		#end event
	#end frameUpdate
...
class Menu:
	...
	def __init__(self):
		pass
	def initialize(self,wnd,ttl="no title", items=None, cursorSound=None, enterSound=None, cancelSound=None, unavailableSound=None):
		"""
	...
	def frameUpdate(self):
		...
		#end arrow keys
		if self.wnd.keyPressed(keyCodes.K_HOME) and self.cursor!=0: self.moveTo(0)
		if self.wnd.keyPressed(keyCodes.K_END) and self.cursor!=len(self.items): self.moveTo(len(self.items)-1)
		...
		if self.wnd.keyPressed(keyCodes.K_PAGEDOWN):
			...
			if n>0: self.moveTo(self.cursor+n)
		#end pagedown
		if self.wnd.keyPressed(keyCodes.K_SPACE): self.moveTo(self.cursor)
	...
	def moveTo(self,c):
		...
		if not self.is_available[c] and self.unavailableSound is not None: playOneShot(self.unavailableSound)
		self.cursor=c
